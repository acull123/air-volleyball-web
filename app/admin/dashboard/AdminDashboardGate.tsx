...
export default function AdminDashboardGate() {
  ...
  useEffect(() => {
    if (!access.loading && !isRole(access.authUser?.profile ?? null, "admin")) {
  ...
  if (!access.authUser || !profile || !isRole(profile, "admin")) {
    return (
      <>
        <PageHero
          eyebrow="Admin Dashboard"
          title="Redirecting"
          description="Returning you to the portal login."
        />
