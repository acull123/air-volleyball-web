...
export default function RegistrationManagerClient() {
  ...
  const effectiveSelectedEventId =
    selectedEventId &&
    registerableEvents.some((event) => event.id === selectedEventId)
      ? selectedEventId
  ...
  const selectedEvent =
    registerableEvents.find((event) => event.id === effectiveSelectedEventId) ??
    null;
  ...
  const selectedPlayer =
    players.data.find(
      (player) => isCurrentPlayer(player) && player.id === selectedPlayerId,
    ) ?? null;
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (
        registrations.data.some(
          (registration) =>
            ...
            registration.playerId === selectedPlayer.id,
        )
      ) {
      ...
      await firestoreApi.registrations.create({
        id: getRegistrationDocumentId(selectedEvent.id, selectedPlayer.id),
      ...
      setError(
        getFriendlyFirebaseError(submitError, "Unable to save registration."),
      );
    } finally {
      setSaving(false);
    }
  ...
  async function updateRegistrationPaymentStatus(
    ...
    try {
      ...
      setError(
        getFriendlyFirebaseError(updateError, "Unable to update registration."),
      );
    } finally {
  ...
  async function deleteRegistration(
    ...
    const confirmed = window.confirm(
      `Remove ${playerName} from this registration?`,
    );
    ...
    try {
      ...
      setError(
        getFriendlyFirebaseError(deleteError, "Unable to delete registration."),
      );
    } finally {
      setUpdatingRegistrationId(null);
    }
              ...
              {selectedEvent ? (
                <div className="rounded-[1.5rem] bg-[color:var(--paper)] px-5 py-5">
                  ...
                  <div className="mt-3 space-y-2 text-sm text-[color:var(--muted)]">
                    <p>
                      Type: {selectedEvent.type === "camp" ? "Camp" : "Tryout"}
                    </p>
                    <p>
                      Date:{" "}
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                {selectedPlayer && (
                  <span className="text-sm text-[color:var(--muted)]">
                    Selected: {selectedPlayer.firstName}{" "}
