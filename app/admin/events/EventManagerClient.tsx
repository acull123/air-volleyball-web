...
const ageGroupOptions = ["12U", "13U", "14U", "15U", "16U", "17U", "18U"];
...
function mapEventToDraft(event: EventDocument): EventDraft {
  const selectedAgeGroups = event.ageGroups?.length
    ? event.ageGroups
    : event.ageGroup
      ? [event.ageGroup]
      : [];
  ...
  return {
    type: isTournamentEventType(event.type) ? "tournament" : event.type,
    tournamentDayCount: String(getTournamentDayCount(event)),
    title: event.title,
...
function formatAgeGroups(event: EventDocument) {
  const ageGroups = event.ageGroups?.length
    ? event.ageGroups
    : event.ageGroup
      ? [event.ageGroup]
      : [];
...
function getGymSpaceLocation(gymSpace: GymSpaceDocument | undefined) {
  return [gymSpace?.facilityName, gymSpace?.location]
    .filter(Boolean)
    .join(" - ");
}
...
function isTournamentType(type: EventDocument["type"]) {
  return isTournamentEventType(type);
}
...
export default function EventManagerClient() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const startTime = formatStoredTime(
        draft.startHour,
        draft.startMinute,
        draft.startMeridiem,
      );
      const selectedGymSpace = activeGymSpaces.find(
        (gymSpace) => gymSpace.id === draft.gymSpaceId,
      );
      const isPractice = draft.type === "practice";
      const isTournament = isTournamentType(draft.type);
      const tournamentDayCount = draft.tournamentDayCount.trim()
        ? Number(draft.tournamentDayCount)
        : 1;
      ...
      const durationMinutes = draft.durationMinutes.trim()
        ...
        : 0;
      const practiceEndDate = getPracticeEndDate(
        draft.startDate,
        startTime,
        durationMinutes,
      );
      const teamSchedules = usesTeams
        ? isPractice
          ? draft.teamSchedules.slice(0, 1)
          : draft.teamSchedules
        : [];
      const payload = {
        type: isTournament ? "tournament" : draft.type,
        tournamentDayCount: isTournament ? tournamentDayCount : 1,
        title: isPractice
          ? getPracticeTitle(teamSchedules, teams.data)
          : draft.title.trim(),
        ...
        ageGroup: isPractice || isTournament ? "" : draft.ageGroups[0] || "",
        ageGroups: isPractice || isTournament ? [] : draft.ageGroups,
        ...
        location:
          isPractice && draft.gymSpaceId
            ? getGymSpaceLocation(selectedGymSpace)
            : draft.location.trim(),
        notes: isPractice ? "" : draft.notes.trim(),
        fullDetails: isPractice ? "" : draft.fullDetails.trim(),
        active: draft.active,
      };
      ...
      if (
        ...
        throw new Error(
          "Title, start date, start time, and location are required.",
        );
      }
      ...
      if (
        ...
        throw new Error("Tournament days must be selected.");
      }
  ...
  async function handleDelete(eventId: string) {
    ...
    try {
      ...
      setError(
        getFriendlyFirebaseError(deleteError, "Unable to delete event."),
      );
    }
  }
  ...
  function toggleAgeGroup(ageGroup: string, checked: boolean) {
    setDraft((current) => {
      const selectedAgeGroups = checked
        ? [...current.ageGroups, ageGroup]
        : current.ageGroups.filter(
            (selectedAgeGroup) => selectedAgeGroup !== ageGroup,
          );
      const ageGroups = ageGroupOptions.filter((option) =>
            ...
            <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit}>
              <label className="flex flex-col gap-2 text-sm font-semibold text-[color:var(--ink)]">
                ...
                >
                  ...
                  <option value="refScoringClinic">
                    Ref And Scoring Clinic
                  </option>
                </select>
              </label>
              {isTournamentType(draft.type) && (
                <>
                  <label className="flex flex-col gap-2 text-sm font-semibold text-[color:var(--ink)]">
                    Days
                    <select
                      value={draft.tournamentDayCount}
                      onChange={(event) =>
                        setDraft((current) => ({
                          ...current,
                  ...
                  <label className="flex flex-col gap-2 text-sm font-semibold text-[color:var(--ink)]">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setDraft((current) => ({
                          ...current,
                          status: event.target.value as EventDocument["status"],
                        }))
                      }
                      className="rounded-2xl border border-[color:var(--line)] px-4 py-3"
                    >
                              ...
                              <input
                                ...
                                onChange={(event) => {
                                  if (draft.type === "practice") {
                                    setDraft((current) => ({
                                      ...current,
                                      teamSchedules: event.target.checked
                                        ? [{ teamId: team.id, scheduleUrl: "" }]
                                        : [],
                                    }));
                                    return;
                                  }
              ...
              {draft.type !== "practice" && (
                <>
                  ...
                  {!isTournamentType(draft.type) && (
                    <div className="md:col-span-2 flex flex-col gap-3 text-sm font-semibold text-[color:var(--ink)]">
                      Age group
                      <div className="grid gap-3 rounded-2xl border border-[color:var(--line)] px-4 py-4 sm:grid-cols-2 lg:grid-cols-4">
                        {ageGroupOptions.map((ageGroup) => (
                          <label
                            key={ageGroup}
                            className="flex items-center gap-3 rounded-2xl bg-[color:var(--paper)] px-3 py-3"
                          >
                            <input
                              type="checkbox"
                              checked={draft.ageGroups.includes(ageGroup)}
                              onChange={(event) =>
                                toggleAgeGroup(ageGroup, event.target.checked)
                              }
                            />
                            <span>{ageGroup}</span>
                          </label>
                        ))}
                        <p className="text-xs font-normal text-[color:var(--muted)] sm:col-span-2 lg:col-span-4">
                          Leave all unchecked for all ages.
                        </p>
                      </div>
                    </div>
                  )}
                ...
                <label className="flex flex-col gap-2 text-sm font-semibold text-[color:var(--ink)]">
                  ...
                  <select
                    ...
                    onChange={(event) => {
                      ...
                      const gymSpace = activeGymSpaces.find(
                        (entry) => entry.id === gymSpaceId,
                      );
              ...
              {draft.type !== "practice" && (
                <>
                  ...
                  <label className="md:col-span-2 flex flex-col gap-2 text-sm font-semibold text-[color:var(--ink)]">
                    Full details
                    <textarea
                      value={draft.fullDetails}
                      onChange={(event) =>
                        setDraft((current) => ({
                          ...current,
                          fullDetails: event.target.value,
                        }))
                      }
                      className="min-h-40 rounded-2xl border border-[color:var(--line)] px-4 py-3"
                      placeholder="Add full descriptions, date ranges, logistics, links, or instructions shown when the event is opened."
                    />
                  </label>
                </>
              )}
                  ...
                  >
                    <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                      <div className="space-y-1">
                        ...
                        <p className="text-sm text-[color:var(--muted)]">
                          Age group: {formatAgeGroups(event)}
                        </p>
                        <p className="text-sm text-[color:var(--muted)]">
                          Dates:{" "}
                        ...
                        <p className="text-sm text-[color:var(--muted)]">
                          External link:{" "}
                          {event.externalUrl ? "Ready" : "Not set"}
                        </p>
