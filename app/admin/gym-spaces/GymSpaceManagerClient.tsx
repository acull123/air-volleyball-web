...
export default function GymSpaceManagerClient() {
  ...
  const [selectedGymSpaceId, setSelectedGymSpaceId] = useState<string | null>(
    null,
  );
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const payload = {
        ...
        startTime: formatStoredTime(
          draft.startHour,
          draft.startMinute,
          draft.startMeridiem,
        ),
        endTime: formatStoredTime(
          draft.endHour,
          draft.endMinute,
          draft.endMeridiem,
        ),
      ...
      setError(
        getFriendlyFirebaseError(submitError, "Unable to save gym space."),
      );
    } finally {
      setSaving(false);
    }
  ...
  async function handleDelete(gymSpaceId: string) {
    ...
    try {
      ...
      setError(
        getFriendlyFirebaseError(deleteError, "Unable to delete gym space."),
      );
    }
  }
                ...
                >
                  <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-[color:var(--muted)]">
                        Days:{" "}
                        {gymSpace.availableDays.length
                          ? gymSpace.availableDays.join(", ")
                          : "No days set"}
                      </p>
                      <p className="text-sm text-[color:var(--muted)]">
                        Window: {formatDisplayTime(gymSpace.startTime)} to{" "}
                        {formatDisplayTime(gymSpace.endTime)}
                      </p>
                      <p className="text-sm text-[color:var(--muted)]">
